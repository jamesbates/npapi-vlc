/*****************************************************************************
 * caopengl.c: OpenGL and OpenGL ES output common code
 *****************************************************************************
...
struct vout_display_caopengl_t {
    vlc_gl_t   *gl;
...
vout_display_caopengl_t *vout_display_caopengl_New(video_format_t *fmt,
                                               const vlc_fourcc_t **subpicture_chromas,
                                               vlc_gl_t *gl)
{
	//printf("vout_display_opengl_New: start\n");
	//msg_Dbg(gl, "vout_display_opengl_New: start\n");
    vout_display_caopengl_t *vgl = calloc(1, sizeof(*vgl));
    if (!vgl)
        return NULL;
    ...
    //printf("vout_display_opengl_New: step 1\n");

    const char *extensions = (const char *)glGetString(GL_EXTENSIONS);
    //printf("vout_display_opengl_New: first GL call completed\n");
    /* Load extensions */
...
#   endif
    ...
    *fmt = vgl->fmt;
	...
	//printf("vout_display_opengl_New: end\n");
    return vgl;
}

void vout_display_caopengl_Delete(vout_display_caopengl_t *vgl)
{
    /* */
    if (!vlc_gl_Lock(vgl->gl)) {
...
picture_pool_t *vout_display_caopengl_GetPool(vout_display_caopengl_t *vgl, unsigned requested_count)
{
    if (vgl->pool)
        return vgl->pool;
...
int vout_display_caopengl_Prepare(vout_display_caopengl_t *vgl,
...
int vout_display_caopengl_Display(vout_display_caopengl_t *vgl,
