 ...
 * $Id: d02af2c18e2e44f6f3eb2073cd6adbded26885d7 $
 *
...
#define CHROMA_TEXT ("Dummy image chroma format")
...
vlc_module_begin ()
    set_shortname( ("MacOSX CAOpenGL vout") )
    set_description( ("Mac OS X CoreAnimation OpenGL video output") )
...
{
	printf("CAOpenGL video: Open start\n");
	...
	id layer = var_CreateGetAddress (vd, "drawable-nsobject");
	if (layer) {

		printf("CAOpenGL video: Found a drawable-nsobject\n");
		[(VlcPluginLayer *)layer sayHi];
	} else {

		printf("CAOpenGL video: No drawable-nsobject found\n");
	}
    ...
    printf("CAOpenGL video: Open end\n");
    return VLC_SUCCESS;
}
...
{
	printf("CAOpenGL video: Close start\n");
    vout_display_t *vd = (vout_display_t *)object;
    ...
    free(sys);
    printf("CAOpenGL video: Close end\n");
}
